#ifndef LATEX_LANGUAGE_SERVER_URI_H
#define LATEX_LANGUAGE_SERVER_URI_H
...
// This class is largely an adaption of https://github.com/Microsoft/vscode-uri/blob/master/src/index.ts
enum CharCode {
    Null = 0,
    /**
     * The `\t` character.
...
private:
    ...
    static string referenceResolution (const string &scheme, const string &path);
...
public:
    ...
    const string getFragment ();

    void print () const;

    string fsPath ();

    static string makeFsPath (const Uri &uri);
